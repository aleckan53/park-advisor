...
function renderSection(obj) {
    ...
    if (obj.data.length > 1) {
        obj.data.forEach(park => {
            $('#parks-list').append(`
                <li id="park-${park.parkCode}">
                    <h3>${park.fullName}</h3>
                    <input type="checkbox">
                    <p>${park.description.substring(0, 100) + "..."}</p>
                    <img src="https://img.sunset02.com/sites/default/files/styles/marquee_large_2x/public/image/2017/04/main/el-capitan-getty-0517.jpg?itok=vuJS4hXh" alt="${park.name}"> 
